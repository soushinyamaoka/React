...
type Rt = RouteProp<DevicesStackParamList, 'ConsumableForm'>;

const ConsumableFormScreen: React.FC = () => {
  const route = useRoute<Rt>();
  const navigation = useNavigation<any>();
  const qc = useQueryClient();
  const { deviceId, consumableId } = route.params;
  ...
  useEffect(() => {
    if (consumableId && deviceQuery.data) {
      const c = deviceQuery.data.consumables.find((x) => x.id === consumableId);
      if (c) {
        setName(c.name);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return createConsumable(deviceId, input as any);
    },
    onSuccess: () => {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <TextField label="消耗品名 *" value={name} onChangeText={setName} placeholder="例: 空気清浄機フィルター" />
        ...
        <TextField label="交換目安" value={replacementIntervalText} onChangeText={setReplacementIntervalText} placeholder="例: 1年" />
        <DateField label="最終交換日" value={lastReplacedDate} onChange={setLastReplacedDate} />
        <DateField label="次回交換予定日" value={nextReplacementDate} onChange={setNextReplacementDate} />
        <TextField label="購入URL" value={purchaseUrl} onChangeText={setPurchaseUrl} autoCapitalize="none" />
        <TextField label="メモ" value={memo} onChangeText={setMemo} multiline />
        <Button title={consumableId ? '更新する' : '追加する'} onPress={onSubmit} loading={mutation.isPending} />
      </ScrollView>
