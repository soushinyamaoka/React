  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Card>
        ...
        <Text style={styles.householdName}>家庭: {user?.householdName} ({user?.role})</Text>
      </Card>

      <MenuItem icon="apps" label="カテゴリ管理" onPress={() => navigation.navigate('CategoryManage')} />
      <MenuItem icon="location" label="設置場所管理" onPress={() => navigation.navigate('LocationManage')} />
      <MenuItem icon="download" label="データエクスポート (JSON)" onPress={() => navigation.navigate('Export')} />
