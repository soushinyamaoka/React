...
import { locationInputSchema } from '@homegear/shared';
...
const locationRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.put('/:id', async (req, reply) => {
    ...
    const exists = await prisma.location.findFirst({ where: { id, householdId: req.auth.householdId } });
  ...
  app.delete('/:id', async (req, reply) => {
    ...
    const exists = await prisma.location.findFirst({ where: { id, householdId: req.auth.householdId } });
    if (!exists) return reply.code(404).send({ message: '設置場所が見つかりません' });
    await prisma.device.updateMany({ where: { locationId: id }, data: { locationId: null } });
