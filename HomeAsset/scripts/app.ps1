...
function Do-Start {
  ...
  if ($Target -eq 'local') {
    ...
    Start-ServiceWindow -Title 'HomeAsset Expo (local)' -NpmScript 'mobile:start' -PidFile $ExpoPidFile -EnvVars @{ EXPO_PUBLIC_API_TARGET = 'local' }

    Write-Host ''
    Write-Host '起動しました。[local モード] アプリはローカル API（PCのLAN IP:4001）を参照します。' -ForegroundColor Green
    ...
    Start-ServiceWindow -Title 'HomeAsset Expo (vps)' -NpmScript 'mobile:start' -PidFile $ExpoPidFile -EnvVars @{ EXPO_PUBLIC_API_TARGET = 'vps' }
